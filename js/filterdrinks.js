 ...
 */
/**
 * Created by ludwing on 2016-03-11.
 */
...
$(document).ready(function () {
        ...
        $.each(drinkList, function (i, data) {
            if (data.namn != ""){
...
$(document).ready(function () {
    ...
    $.each(drinkList, function (i, data) {
        if (data.glu){
            $('.productwine').append("<div id='" + data.beer_id + "' class='alcohol' draggable='true' ondragstart='drag(event)' ondragend='dragEnd(event)'>" +
                "<img src='images/beer.svg' alt='dricka' draggable='false'>" +
                "<p id='bearBrand'>Name: " + data.namn + "</p>" +
                "<p id='bearPrice'><b id='price'>Price: </b>" + data.pub_price + " kr</p>" +
                "</div>");
